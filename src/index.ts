 ...
 */
interface DirectoryEntry extends FileStats {
 ...
 */
interface FileContent extends FileStats {
  content: string;
}
 ...
 */
async function getAllFiles(dirPath: string, pattern?: string): Promise<string[]> {
 ...
 * Calculate file hash
 */
async function calculateHash(filePath: string, algorithm: string = 'sha256'): Promise<string> {
 ...
 * Analyze text file
 */
async function analyzeTextFile(filePath: string): Promise<TextAnalysis> {
 ...
 */
async function searchInFile(filePath: string, pattern: RegExp): Promise<SearchMatch[]> {
  const content = await fs.readFile(filePath, 'utf8');
  const lines = content.split('\n');
 ...
 */
async function findDuplicates(dirPath: string): Promise<DuplicateGroup[]> {
 ...
 */
async function listDirectoryContents(dirPath: string, recursive: boolean): Promise<DirectoryEntry[]> {
 ...
 */
async function readFileWithMetadata(filePath: string, encoding: BufferEncoding = 'utf8'): Promise<FileContent> {
